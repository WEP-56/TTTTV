...
impl SourceCrawler for DefaultSourceCrawler {
    async fn search(
        ...
        let url = Url::parse_with_params(base_url, &[("ac", "videolist"), ("pg", "1"), ("wd", keyword)])
            .map_err(|e| MoovieError::SourceSearchError(format!("URL解析失败: {}", e)))?;
        ...
        for item in list {
            ...
            if vod_item.vod_play_url.is_empty() {
                continue;
            }
    ...
    async fn get_detail(
        ...
        let url = Url::parse_with_params(base_url, &[("ac", "detail"), ("ids", vod_id)])
            .map_err(|e| MoovieError::DetailError(format!("URL解析失败: {}", e)))?;
        ...
        if vod_item.vod_play_url.is_empty() {
            return Err(MoovieError::DetailError("没有播放链接".to_string()));
        }
